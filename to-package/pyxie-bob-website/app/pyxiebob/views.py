...
# This is all of the views that the PyxieBob application supports
...
from pyxiebob.models import Program, Tutorial, UserProfile, Version, FacilitatorRequest
import re
import os
...
def create_program(request):
    return render(
        request, 'pyxiebob/create_program.html',
...
def create_user_form(request):
    return render(
        request, 'pyxiebob/create_user.html',
        _add_defaults(request, {})
    )
...
def programs(request):
    ...
    if not use_cache:
        ...
        result = render(
            request, 'pyxiebob/programs.html',
            _add_defaults(request,{'programs':programs})
        )
        now = time.time()
...
def tutorial(request, tutorial_name, page_number=1):
    ...
    return render(
        request, 'pyxiebob/tutorial.html',
...
def tutorials(request):
    ...
    return render(
        request, 'pyxiebob/tutorials.html',
...
def register_user(request):
    return render(
        request, 'pyxiebob/register_user.html',
        _add_defaults(request)
    )
...
def login_pane(request):
    return render(
        request, 'pyxiebob/partials/_login_pane.html',
        _add_defaults(request)
    )
