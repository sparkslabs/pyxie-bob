...
function enablePageInteraction() {
    ...
    if (create_program_btn.length) {
        ...
        create_program_btn.click(function () {
            ...
            compileNewProgram(
                function(program_id) {
                    window.location.replace('/pyxiebob/program/'+program_id);
            });
        })
    } else {
        ...
        forkCodeBtn.click(function(ev) {
            ...
            bootbox.confirm(
                ...
                function(result) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "/pyxiebob/fork_code",
                        data: JSON.stringify({
                            "src_id": $(ev.currentTarget).attr('data-src-id')
                        }),
                        success: function(data) {
                            data = JSON.parse(data);
                            window.location.replace('/pyxiebob/program/'+data['fork_id'])
                        },
                        error: function (jqXhr, textStatus, errorThrown) {
        ...
        facilitator_password_reset_request_btns.click(function(ev) {
            ...
            bootbox.confirm("<p>This will reset the password for this user, and it's your responsibility to pass the new password along to them.</p><p><em>Are you sure?</em></p>", function(result) {
                if (result) {
                    $.ajax({
                        type: "POST",
                        url: "/pyxiebob/confirm_password_reset",
                        data: JSON.stringify({
                            "id": child_id
                        }),
                        success: function(data) {
                ...
                buttons: {
                    make_request: {
                        ...
                        callback: function() {
                            $.ajax({
                                type: "POST",
                                url: "/pyxiebob/request_password_reset",
                                data: JSON.stringify({
...
function setupUserDetailsUpdate() {
    if (update_user_details_btn.length > 0 ) {
        ...
        update_user_details_btn.click(function() {
            ...
            $.ajax({
                type: "POST",
                url: "/pyxiebob/update_user_details/",
                data: JSON.stringify({
                    "name": $('#name').val(),
                    "email": email,
                    "question_answers": [
...
function respondToFacilitatorRequest(request_id, accepted) {
    ...
    $.ajax({
        type: "POST",
        url: "/pyxiebob/respond_to_facilitator_request",
        data: JSON.stringify({
...
function makeFacilitorRequest(faciliator_name) {
    ...
    $.ajax({
        type: "POST",
        dataType: 'text',
        url: "/pyxiebob/facilitator_request",
        data: JSON.stringify({
...
function signOut() {
    $.ajax({
        type: "GET",
        url: "/pyxiebob/sign_out",
        success: function(data) {
            //updateLoginForm();
            location.replace("http://pyxiebob.iotoy.org/pyxiebob/");
            // location.reload();
        }
    })
}
    ...
    $.ajax({
        ...
            });
            ...
            $('#loginSubmit').click(function(ev) {
                ...
                $.ajax({
                    type: "POST",
                    url: "/pyxiebob/authenticate_user/",
                    data: JSON.stringify({
                        "username": username,
                        "password": password
                    }),
...
function updateProgramStatus(program_id) {
    ...
    $.ajax({
        type: 'GET',
        url: '/pyxiebob/queue_status/'+program_id,
        timeout: 5000,
        success: function(data) {
            data = JSON.parse(data);
...
function statusElementContent(data) {
    if (data.status == 'no_version') {
        ...
        return(
            '<a class="btn btn-primary" href="/pyxiebob/download/'+data.id+'">' +
...
function setupBlockly() {
    if (document.getElementById('blockly')) {
		...
		$(document).ready(function() {
            ...
            var jqxhr = $.get('/static/pyxiebob/js/dal_interpreter.txt', function(data, txt, err) {
                dalcode = data;
