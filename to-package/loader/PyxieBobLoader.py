...
class PyxieBobLoader(QtGui.QWidget):

    def __init__(self):
        super(PyxieBobLoader, self).__init__()
        self.initUI()

    def initUI(self):
        self.btn = QtGui.QPushButton('PROGRAM PYXIEBOB!', self)
        self.btn.move(20, 20)
        self.btn.clicked.connect(self.showDialog)
        self.btn.setGeometry(300, 340, 180, 30)

        pixmap = QtGui.QPixmap("pyxiebob-small.jpg")

        palette = QPalette()
        palette.setBrush(QPalette.Background,QBrush(pixmap))

        self.setPalette(palette)
        self.setWindowTitle('PyxieBob Loader')
    ...
    def checkHexfileIsPyxieBobFile(self): # This could be checked by looking for a magic string sequence
        return True
